...
void TreeView::Draw(DrawingContext& drawingContext)
{
    ...
    const Color backgroundColor = {15, 90, 100, 225};
    const auto width = static_cast<float>(horizontalLayout->GetWidth());
    const auto height = static_cast<float>(horizontalLayout->GetHeight());

    primitiveBatch->DrawRectangle(globalTransform * horizontalLayout->GetTransform(), Vector2::Zero, width, height, backgroundColor);
#endif
