...
SpriteFontTest::SpriteFontTest(const std::shared_ptr<GameHost>& gameHostIn, const std::shared_ptr<vfs::FileSystemContext>& fs)
    : gameHost(gameHostIn)
    , fs_(fs)
...
void SpriteFontTest::draw()
{
    ...
    spriteFont->draw(*spriteBatch, text, Vector2::createZero(), Color::createLime(), math::toRadian(-90.0f), Vector2{0.0f, 0.0f}, 1.0f);
